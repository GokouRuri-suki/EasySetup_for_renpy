...
screen say(who, what):
    ...
    if not renpy.variant("small"):
        add SideImage() yalign 1.0 xalign 0
    #——---------------------------------------------------
...
#------------ 我把它注释掉了 改为右上角的两个imagesbutton

#####################################仿作BA右上角ui#################

# 定义一个专门放右上角按钮的屏幕
screen topRight_button_screen():
    # zorder 100 确保它显示在最上层，不会被立绘或对话框遮挡
    zorder 100 
    hbox:
        xalign 0.99
        yalign 0.03
        ...
        imagebutton:
            idle            "auto_idle"
            selected_idle   "auto_selected"
            selected_hover  "auto_selected"
           
            action Preference("auto-forward", "toggle")  
            # xalign 0.95                          # 靠右 (0.0是左, 1.0是右)
            # yalign 0.02                          # 靠上 (0.0是顶, 1.0是底)
            # 如果按钮之间有重叠，可以调整 xoffset 或 yoffset

        # --- 第二个按钮 菜单 ---
        imagebutton:
            idle "menu_idle"      # 替换为你的图片路径
            #hover 
            action ShowMenu("preferences")                     # 点击动作：回退剧情
            # xalign 0.88                          # 在第一个按钮的左边一点
            # yalign 0.02
...
#主界面的菜单排列
# screen Big_Text():
#     text "_____这个是logo_____"size 60 xpos 0.4 ypos 0.4
screen main_menu():
    ...
    add "gui/logo.png":
        xanchor 0.5   
        yanchor 0.5
        xpos 0.5    #改这里 这个是居中x的中心
        ypos 0.3    #改这里 这个是居于y的上3/10的位置
